            ...
            <div className="relative group">
               ...
               <div className="absolute top-full right-0 mt-2 w-48 bg-card border border-border rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all py-2 z-50">
                   <div className="px-4 py-2 border-b border-border mb-1">
                     <p className="text-[11px] font-bold truncate">{user.fullName || user.email?.split('@')[0]}</p>
                     <p className="text-[9px] text-muted-foreground truncate">{user.email}</p>
                   </div>
                   
                   <div className="px-2 py-1 flex flex-col gap-1 border-b border-border/40 mb-1">
                     <Link href="/profile" className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-bold text-foreground hover:bg-muted rounded-md transition-all">
                       <User className="w-3.5 h-3.5 text-primary" />
                       Candidate Hub
                     </Link>
                     <Link href="/employer" className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-bold text-foreground hover:bg-muted rounded-md transition-all">
                       <svg stroke="currentColor" fill="none" strokeWidth="2" viewBox="0 0 24 24" strokeLinecap="round" strokeLinejoin="round" className="w-3.5 h-3.5 text-indigo-500" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="7" width="20" height="14" rx="2" ry="2"></rect><path d="M16 21V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"></path></svg>
                       Employer Portal
                     </Link>
                   </div>
                   ...
                   <div className="px-2 pb-1">
                     <button 
                       ...
                       className="w-full flex items-center gap-2 px-3 py-1.5 text-[11px] font-bold text-rose-500 hover:bg-rose-50 rounded-md transition-all"
                     >
